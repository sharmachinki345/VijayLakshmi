...
const Gallery = () => {
  ...
  return (
    <div className="flex flex-col items-center py-16 min-h-screen md:px-10 lg:px-20">
      <h1 className="text-[#2F2F2F] text-3xl md:text-4xl lg:text-5xl  font-bold ">Gallery</h1>
        ...
        {images.map((image, index) => {
          ...
          return (
            <div key={index} className={`flex-initial ${isLarge} `}>
              <img src={image.src} alt={`Gallery item ${index + 1}`} className="w-full h-full rounded-lg shadow-lg border-2 border-black" />
            </div>
          );
        })}
      </div>
